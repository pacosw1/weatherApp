...
import { Box, Subtitle, Header, Text } from "pakinski-ui";
...
const Dashboard = (props) => {
  ...
  useEffect(() => {
    ...
    if (forecast["forecast"]) {
      ...
      drawChart2(mobile ? w : w / 2, forecastDays[1].hour, 2);
      drawChart2(mobile ? w : w / 2, forecastDays[2].hour, 3);
    }
  }, [w, forecast]);
      ...
      <ResizeDiv>
        <Search>
          ...
          {cities.length < 1 ? null : (
            <AutocompleteDiv searchBarHeight={4}>
              <Text
                color="black"
                padding={"0 1rem"}
                style={{
        ...
        {forecastDays.length === 0 ? null : (
          <>
            <Text size={2} margin={0} padding={0} mTop={1} mBot={0.4}>
              Next Days
            </Text>
            ...
            <Box direction="col">
              <Box
                direction={"row"}
                style={{ overflowX: mobile ? "scroll" : "hidden" }}
              >
                <Box
                  direction="col"
...
const AutoCompleteCity = ({ name, country, last, id, onGetWeatherData }) => {
  return (
    <Box
      bgColor="white"
      direction="row"
      hSpacing="space-between"
      vSpacing="center"
      padding="1rem 1rem"
      bBot={last ? null : "1px solid #dddd"}
      last={last}
      onClick={() => onGetWeatherData(id)}
      style={{
        cursor: "pointer",
        "&hover:": {
          backgroundColor: "#ddd",
    ...
    >
      <Text margin={"0"} padding="0" size={0.8}>
        {name}
      </Text>
      <Text size={0.8}>{country}</Text>
    </Box>
  );
};
